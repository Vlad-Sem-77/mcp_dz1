...
5. Для Trivago (search_hotels) сначала автоматически вызывается двухшаговый поиск (это уже реализовано внутри инструмента) — тебе достаточно передать английское название города и даты заезда/выезда в формате YYYY-MM-DD.
...
TOOLS_SCHEMA = [
    ...
    },
    {
        ...
        "function": {
            "name": "search_hotels",
            "description": (
                "Ищет отели через Trivago MCP (двухшаговый поиск: сначала "
                "suggestions, затем accommodation-search — оба шага выполняются "
                "автоматически внутри этого инструмента). Город должен быть на английском."
            ),
            "parameters": {
                "type": "object",
                "properties": {
...
class TravelAgent:
    ...
    def _execute_tool(self, name: str, arguments: dict) -> dict:
        ...
        if name == "search_hotels":
            return self._search_hotels_two_step(
    ...
    def _search_hotels_two_step(self, city: str, checkin: str, checkout: str, adults: int) -> dict:
        suggestions = self.trivago.search_suggestions(city)
        if "error" in suggestions:
            return suggestions

        location_id, ns = self.trivago.extract_id_ns(suggestions)
        if not location_id or not ns:
            return {"error": f"Не удалось найти идентификатор локации для города '{city}' в Trivago"}

        return self.trivago.accommodation_search(
